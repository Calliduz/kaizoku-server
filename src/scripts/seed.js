...
async function runSeed() {
  try {
    ...
    logger.info("[Seeder] Clearing entire database as requested...");
    await Anime.deleteMany({});
    const Episode = require("../models/Episode");
    await Episode.deleteMany({});
    logger.info("[Seeder] Database fully cleared!");
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const animeData = {
        ...
        episodeDuration: item.duration?.value
          ? Math.round(item.duration.value / 60)
          : null,
        rating: item.score?.arithmeticMean
          ? Math.round(item.score.arithmeticMean * 10)
          : 0,
    ...
    logger.info(
      `[Seeder] Seeding done! Successfully processed ${upseretCount} items.`,
    );

    logger.info(
      "[Seeder] Starting catalog scrape to prioritize high-quality AniList metadata for trending series...",
    );
    const { scrapeCatalog } = require("../scrapers/engine");
    await scrapeCatalog({ fetchEpisodes: true, maxPages: 2 });
    const puppeteerPool = require("../utils/puppeteerPool");
    await puppeteerPool.shutdown();
    logger.info("[Seeder] Database fully rebuilt and seeded.");
  } catch (err) {
